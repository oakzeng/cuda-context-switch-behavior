...
# Usage: ./run-faulter-faulter.sh [MiB] [stride_bytes] [iters] [use_managed_memory] [use_mps]
# Example: ./run-faulter-faulter.sh 4096 65536 5 1 1
...
nsys profile \
  -o faulter-faulter-MPS_$MPS-$(date +%Y%m%d_%H%M%S) \
